...
export default function Dark() {
  ...
  const getTopLinks = async () => {
    try {
      setTopLinks(await getDiscord("834797161081602088"))
    } catch {
      setError(true)
    }
  }
  ...
  const getBulletinBoard = async () => {
    try {
      setBulletinBoard(await getDiscord("932043003055456336"))
    } catch {
      setError(true)
    }
  }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 divide-x overflow-hidden divide-zinc-800 bg-[#36393f] text-white font-discord min-h-screen">
        {error && (
          <div className="md:col-span-3 p-5 text-sm text-zinc-300">
            Discord messages are unavailable right now.
          </div>
        )}
